...
function renderTable() {
  ...
  if (!rows.length) {
    tableSlot.innerHTML = `<div class="empty-state">Aucune candidature ne correspond à votre recherche.</div>`;
    return;
  }
  ...
  rows.forEach((a) => {
    ...
    tr.innerHTML = `
      ...
      <td>${a.email ? `<a href="mailto:${escapeHtml(a.email)}">${escapeHtml(a.email)}</a>` : "—"}</td>
      <td class="note-cell">${a.note ? escapeHtml(a.note) : "—"}</td>
      <td>
