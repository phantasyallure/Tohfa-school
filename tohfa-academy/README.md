...
### Enabling section background photos ("Apparence du site")

The admin panel has an **Apparence du site** page where you can upload a
background photo for the hero banner, each of the three category cards
(Académique / Artistique / Langues) and the "Votre avenir commence ici"
banner. A dark overlay is applied automatically on top of every photo so
the existing text always stays readable, whatever the photo looks like.

To enable it, run this once in the Supabase **SQL Editor** (in addition to
the script above):
...
create policy "admins can write site settings" on site_settings
  for all to authenticated using (true) with check (true);
```

Then create a public storage bucket for the photos: in Supabase go to
**Storage → New bucket**, name it exactly `site-images`, and mark it
**Public**. Admins (anyone who can log in at `/admin`) can then upload,
replace or remove each section's photo from **Apparence du site** — no
code changes needed.

### Language switcher

The header's globe icon opens a dropdown to switch between **Français**,
**العربية** (with the Algerian flag) and **English**. It closes when a
language is picked, when clicking anywhere else on the page, or on
<kbd>Escape</kbd>.
...
You have two ways to provide your Supabase credentials — pick one.

**Option A — edit the file directly (simplest).** Open `js/supabase-config.js`
and fill in your project's values (found in **Project Settings → API** in
Supabase):
...
**Option B — use Vercel environment variables (keeps values out of git).**
This project already includes a small build step for this. In your Vercel
project, go to **Settings → Environment Variables** and add exactly these
two names (apply them to Production and Preview):
...
No `NEXT_PUBLIC_` or `VITE_` prefix — that convention is specific to
Next.js/Vite projects, not this one. At build time, `scripts/generate-config.js`
reads these two variables and writes them into `js/supabase-config.js`
automatically (via the `vercel-build` script in `package.json`), so the
real values never need to be committed to your repository. If you don't
set these variables, Vercel just leaves the file as you last edited it —
so Option A and Option B never conflict.

Either way, only ever use the **anon / public** key — never the
`service_role` key, which must stay private.
